...
namespace Library.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult searchBook(string Key = "")
        {
            if (HttpContext.Session.GetInt32("id") != null)
            {
                List<Book> books = context.Books.Where(m => m.bookTitle.Contains(Key)).OrderBy(m => m.BookId).ToList();
                ViewBag.allBooks = books;
                List<Author> authors = context.Authors.ToList();
                ViewBag.allAuthor = authors;
                return View("Books");
        ...
        public IActionResult searchAuthor(string Key = "")
        {
            if (HttpContext.Session.GetInt32("id") != null)
            {
                List<Author> authors = context.Authors.Where(s => s.authorName.Contains(Key)).OrderBy(m => m.AuthorId).ToList();
                ViewBag.allAuthors = authors;
                return View("Authors");
            }
            return RedirectToAction("Login");
